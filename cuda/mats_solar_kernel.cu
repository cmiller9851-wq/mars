#include "../include/mars_solar_kernel.cuh"
...
__global__ void __launch_bounds__(256, 2) mars_solar_throughput_kernel_v2(
    const float* __restrict__ dem_grid,
    float* __restrict__ irradiance_out,
    const int width,
    const int height,
    ...
    // Parallel halo boundary prefetching
    if (tx == 0) {
    ...
    // Horizon cutoff evaluation
    if (params.sz <= 0.0f) {
        irradiance_out[idx] = 0.0f;
        return;
    }

    // 3x3 finite difference stencil calculation
...
extern "C" {
    void launch_mars_solar_throughput(
        ...
        mars_solar_throughput_kernel_v2<<<grid, block, 0, stream>>>(
            d_dem_grid, d_irradiance_out, width, height, params
        );
    }
}
